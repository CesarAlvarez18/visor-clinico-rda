...
- **Silueta del cuerpo humano (vista principal)**: regiones por sistema coloreadas según el nivel de afectación; hover muestra resumen, clic abre el detalle del sistema. Selector de fecha / atención para ver el estado en cada momento. **Estilo visual (decidido 2026-09-19)**: toda la aplicación en **tema oscuro** tipo "radiografía" (tokens en `src/index.css`): azul petróleo con acentos cian, cuerpo translúcido con borde cian brillante, anatomía decorativa (columna, costillas, pelvis, grandes vasos, pliegues cerebrales, asas intestinales) en cian tenue y órganos con resplandor del color de su nivel. "Sin datos" es el trazado cian con trama, sin color de nivel. El **árbol vascular** (aorta, carótidas, arterias de brazos y piernas) se pinta con el color del nivel del sistema cardiovascular, así que solo es rojo cuando ese sistema está en "Grave". Los rótulos de nivel van en los márgenes del lienzo con línea guía al órgano, para no tapar los órganos. **Mejora visual (2026-09-19)**: borde tipo Fresnel (filtro SVG `body-rim`: halo interior, centro translúcido), órganos con reflejo de "gel", esqueleto axial dibujado por encima de los órganos, barrido holográfico lento (se apaga con `prefers-reduced-motion`) y, en el sistema musculoesquelético, el nivel se pinta sobre los **huesos largos** de brazos y piernas (`strokes` en `bodyRegions.ts`) en vez de rellenar la extremidad; en "Sin datos" los huesos van en cian tenue. **Estilo "render anatómico" (2026-09-19, reemplaza el azul petróleo dentro del lienzo de la silueta)**: fondo azul profundo, cuerpo azul translúcido en **posición anatómica** (brazos separados, palmas al frente, manos con dedos), relieve muscular tenue y **esqueleto color hueso**: cráneo con cuencas, vértebras y arco posterior de las costillas detrás de los órganos; clavículas, costillas, esternón y pelvis delante. Los **nervios periféricos** se pintan con el color del nivel del sistema nervioso, igual que los vasos con el cardiovascular; sin datos ambos usan tonos neutros (`TREE_NO_DATA`), nunca rojo ni amarillo fijos, para no confundirlos con la escala de niveles. El color hueso tampoco pertenece a la escala. Paleta en `src/components/BodyMap/levelStyles.ts`. La silueta se dibuja a mano solo del lado izquierdo (`bodyRegions.ts`) y el derecho se genera en espejo con `svgPath.ts`.
...
- **Fase**: segundo feature completado y verificado el 2026-09-20: modelo anatómico 3D conectado al RDA (spec `docs/specs/2026-09-20-modelo-3d-rda.md`, plan `docs/plans/2026-09-20-modelo-3d-rda.md`). Antes, el 2026-09-19: silueta del cuerpo interactiva (spec `docs/specs/2026-09-19-silueta-cuerpo.md`, plan `docs/plans/2026-09-19-silueta-cuerpo.md`). Siguientes candidatos: tarjetas (medicamentos, alergias, procedimientos), arrastre de diagnósticos crónicos vía RDA de paciente, validación clínica de las reglas y de la tabla anatómica del modelo 3D.
- **Stack (prototipo)**: **React 19 + TypeScript + Vite 8**, todo en el navegador (sin backend). Pruebas con **Vitest 5** + Testing Library (jsdom); lint con **oxlint** (el que trae la plantilla de Vite). Tipos FHIR con `@types/fhir`. Silueta 2D en **SVG propio**; modelo 3D con **React Three Fiber + Drei + postprocessing** (`three` 0.186, **React fijado en `~19.2`** porque R3F 9.7 exige `<19.3`), cargado en diferido para no pesar en el bundle principal. Backend, autenticación y auditoría se agregarán cuando se salga de datos sintéticos.
- **Capas en `src/`**: `domain/` (tipos del dominio), `fhir/` (lectura y validación de Bundles), `rules/` (mapeos, configuración y cálculo de nivel como funciones puras, para poder moverlas a un backend después), `state/` (estado de la aplicación), `components/BodyMap/` (silueta 2D), `components/Anatomy3D/` (vista 3D), `components/` (panel, selector, encabezado, conmutador 2D/3D), `data/examples/` (Bundles sintéticos), `test/` (setup de pruebas).
- **Comandos**: `npm run dev` (servidor en http://localhost:5173), `npm run build`, `npm run build:model` (arma `public/models/anatomy.glb` desde BodyParts3D, ver más abajo), `npm test` (Vitest, una pasada), `npm run test:watch`, `npm run lint`. En Claude Code el servidor se levanta con la configuración `visor` de `.claude/launch.json`.
...
- **Vista 3D conectada al RDA (2026-09-20, reemplaza la demostración `/?vista=3d`)**: conmutador **2D / 3D** en `App.tsx` (`ViewModeToggle`), por defecto en 2D, conserva paciente/atención/sistema seleccionado al cambiar de vista. `BodyModel3D` (`src/components/Anatomy3D/`) recibe el mismo contrato de props que `BodyMap` (`{ bodyState, selectedSystemId, onSelect }`), se carga en diferido (`lazy`) y no descarga three.js ni el modelo hasta que el médico pulsa "3D". **Contrato de nombres del modelo**: un nodo por grupo — `skin`, `skeleton`, y `system:<BodySystemId>` para cada uno de los 9 sistemas con estructura (`classifyMesh.ts`); los huesos largos van en `system:musculoskeletal`, el resto del esqueleto en `skeleton`. La anatomía procedural (`proceduralAnatomy.ts`) sigue el mismo contrato y es el respaldo si el modelo real no está disponible o el equipo no soporta 3D. Materiales por nivel en `levelMaterials.ts` (nunca un color de la escala en "Sin datos"); interacción (hover/clic/rótulos) en `AnatomyModel.tsx` y `SystemLabels.tsx`; lista accesible de los 9 sistemas en `SystemList.tsx` (CA-19/20). Esqueleto y órganos semitranslúcidos con `depthWrite: false` para no taparse entre sí.
  - **Modelo real**: generado con `npm run build:model` (`scripts/build-anatomy-glb.mjs`) a partir de **BodyParts3D** (DBCLS, Japón), árbol **PART-OF** (no el IS-A: solo el PART-OF resuelve, vía `partof_element_parts.txt`, qué mallas finas `FJ####.obj` unir para reconstruir un órgano completo). Tabla parte → grupo en `scripts/anatomy-parts.mjs`, documentada con origen, versión y limitaciones en `docs/reference/bodyparts3d.md`. Pipeline con `@gltf-transform/*` + `meshoptimizer`: une mallas por grupo, suelda, simplifica (menos agresivo en vasos y nervios), comprime con **Meshopt** (sin Draco, para que Drei lo decodifique sin depender de un CDN externo). `public/models/anatomy.glb` resultante: ~4.3 MB.
  - **Licencia (corrección sobre lo dicho antes)**: BodyParts3D **ya no es CC BY-SA 2.1 Japón**; la fuente oficial (actualizada 2025-02-25) dice **CC BY 4.0** (solo atribución, sin "compartir igual"). Esto resuelve la duda de si la licencia era compatible con distribuir el visor. Atribución en `public/models/LICENSE.md` y en el pie de la vista 3D.
  - **Limitaciones conocidas del modelo real** (del conjunto de datos, no del mapeo): sin tiroides completa, sin nervios periféricos de extremidades, sin útero/ovario/testículo (BodyParts3D modela un único adulto varón — el sistema reproductor se representa solo con la próstata, como región pélvica genérica) y sin esqueleto propio del pie. Detalle en `docs/reference/bodyparts3d.md`.

## Decisiones pendientes
- [ ] Quién valida clínicamente las reglas, el mapeo CIE-10 y los rangos, y la tabla parte anatómica → sistema del modelo 3D (`scripts/anatomy-parts.mjs`).
...
- [ ] Modelo 3D femenino: BodyParts3D solo trae un adulto varón; ¿se necesita pronto uno femenino, o el modelo genérico basta para v1?
- [ ] ¿La vista 3D debe recordar la preferencia 2D/3D del médico entre sesiones, o siempre abrir en 2D?
- [ ] ¿Se agrega una capa muscular tenue al modelo 3D para acercarse más a la imagen de referencia, o se deja por el peso que agregaría?
